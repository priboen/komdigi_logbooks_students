...
class InternshipRemoteDatasources {
  ...
  Future<Either<String, PersonalInternshipResponseModel>> personalInternship({
    ...
    try {
      ...
      final url = Uri.parse('${Variables.baseUrl}/api/internships/join');
      final request = http.MultipartRequest('POST', url)
        ..headers.addAll(headers)
        ..fields['leader_email'] = email
      ...
      final body = await response.stream.bytesToString();
      if (response.statusCode == 200) {
        return Right(PersonalInternshipResponseModel.fromJson(body));
      } else {
  ...
  Future<Either<String, String>> deleteInternship(
    int? id,
  ) async {
    final authData = await AuthLocalDatasource().getAuthData();
    final headers = {
    ...
    final url = Uri.parse('${Variables.baseUrl}/api/internships/$id');
    final response = await http.delete(url, headers: headers);
    if (response.statusCode == 200) {
      return Right('Berhasil menghapus data');
    } else {
